...
def _classifier_prompt(task: str, surface: str, catalog: ModelCatalog) -> str:
    ...
    return (
        ...
        "or high-consequence decisions whose correctness requires substantial reasoning. "
        "Choose Astra when this reasoning depth is needed, including for a single spawned agent. "
        "Do not require another model to fail first, or reserve Astra only for nearly impossible tasks.\n"
        "Do not make Sol the default ceiling for difficult tasks. Higher reasoning effort on Sol "
        "is not a substitute for Astra when the task needs Astra capability. "
        "If Astra fits and an Astra model is listed in Allowed models, select it directly. "
        "If no Astra model is listed, choose the strongest suitable available model and explain "
        "the availability limitation. Assess the actual work, not keywords alone; do not enforce "
        "model quotas or upgrade routine work merely because it mentions architecture or security.\n\n"
        "Reasoning roles:\n"
        "- Select model capability and reasoning effort independently. Astra defaults to medium: "
        "its stronger capability does not imply high or xhigh effort. For most Astra tasks, "
        "including architectural analysis and cross-system debugging, medium is sufficient. "
        "Use high only when the task explicitly needs unusually sustained reasoning beyond "
        "Astra medium; reserve xhigh/max for exceptional proof or search depth. Complexity, "
        "risk labels, or selecting Astra alone do not justify increasing its effort.\n"
        "- low: straightforward; medium: everyday agentic work and the default for Astra; "
        "high: unusually sustained reasoning, subject to the Astra guidance above.\n"
